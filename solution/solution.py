    ...
    def score_response(
        ...
        try:
            ...
            if "scores" in parsed and "reasoning" in parsed:
                ...
                return {"scores": parsed, "reasoning": "Parsed from simple JSON"}
        except Exception:
            default_scores = {criterion: 0.5 for criterion in rubric.keys()}
            return {"scores": default_scores, "reasoning": "Failed to parse"}
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def get_avgs(results_list):
            if not results_list:
                return 0.0, 0.0, 0.0
            f = sum(r.faithfulness for r in results_list) / len(results_list)
            rel = sum(r.relevance for r in results_list) / len(results_list)
            c = sum(r.completeness for r in results_list) / len(results_list)
            return f, rel, c
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        """
        if not failures:
            return ["Review metrics to ensure evaluation is robust.", "Add more challenging cases to dataset.", "Monitor agent performance."]
        ...
        if "irrelevant" in categories:
            suggestions.append("Improve prompt clarity to ensure agent focuses on the user's question.")
        if "off_topic" in categories:
            suggestions.append("Refine intent detection to avoid off-topic responses.")
            
        default_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation.",
            "Add few-shot examples showing complete answers to improve completeness.",
            "Implement hallucination checker to filter unsupported claims.",
            "Improve prompt clarity to ensure agent focuses on the user's question."
        ]
